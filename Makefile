...
.PHONY: help helm-lint helm-template full-check verify-locks validate-secrets \
	...
	test-api-strategy-engine test-api-strategy-sdk test-api-feature-platform test-api-premarket test-api-premarket-catalyst test-api-premarket-gap test-api-premarket-scoring test-api-premarket-morning-briefing test-api-dashboard test-api-human-review test-api-portfolio-service test-api-risk-engine \
	...
	validate-sprint3-release test-sprint3-gate gate-sprint3
...
help:
	...
	@echo "Sprint 11 targets: test-api-human-review"
...
test-api-data-quality:
	@cd "$(API_DIR)" && uv run pytest -q \
		tests/unit/test_data_quality_policy_and_settings.py \
		tests/unit/test_data_quality_engine_rules.py \
		tests/unit/test_data_quality_monitoring.py \
		tests/contract/test_data_quality_contract.py \
		tests/integration/test_data_quality_orchestrator.py

smoke-api-data-quality:
	@cd "$(API_DIR)" && uv run pytest -q tests/smoke/test_data_quality_smoke.py
...
test-api-strategy-sdk:
	@cd "$(API_DIR)" && uv run pytest -q \
		tests/unit/test_strategy_sdk_acceptance_blockers.py \
		tests/unit/test_strategy_sdk_models.py \
		tests/unit/test_strategy_runtime_session.py \
		tests/unit/test_strategy_runtime_replacement.py \
		tests/unit/test_strategy_sdk_downstream_and_adapter.py \
		tests/unit/test_strategy_sdk_state_and_budgets.py \
...
test-api-premarket:
	@cd "$(API_DIR)" && uv run pytest -q \
		...
		tests/unit/test_premarket_gap_engine.py \
		tests/contract/test_premarket_gap_contract.py \
...
test-api-premarket-gap:
	@cd "$(API_DIR)" && uv run pytest -q \
		tests/unit/test_premarket_gap_engine.py \
		tests/contract/test_premarket_gap_contract.py \
		tests/integration/test_premarket_gap_market_data_boundary.py

test-api-premarket-scoring:
	@cd "$(API_DIR)" && uv run pytest -q \
		tests/unit/test_premarket_scoring_engine.py \
		tests/contract/test_premarket_scoring_contract.py \
		tests/integration/test_premarket_scoring_boundary.py
...
test-api-human-review:
	@cd "$(API_DIR)" && uv run pytest -q \
		tests/unit/test_human_review_engine.py \
		tests/contract/test_human_review_contract.py \
		tests/integration/test_human_review_boundary.py
...
test-api-risk-engine:
	@cd "$(API_DIR)" && uv run pytest -q \
		tests/unit/test_risk_models.py \
		tests/unit/test_risk_policy.py \
		tests/unit/test_risk_rules.py \
		tests/unit/test_risk_engine.py \
		tests/contract/test_risk_engine_contract.py \
		tests/integration/test_risk_portfolio_integration.py \
		tests/unit/test_container.py

test-api-order-management:
	@cd "$(API_DIR)" && uv run pytest -q \
		tests/unit/test_order_models.py \
		tests/unit/test_order_transitions.py \
		tests/unit/test_order_aggregate.py \
		tests/unit/test_order_repository.py \
		tests/unit/test_order_service.py \
		tests/contract/test_order_management_contract.py \
		tests/integration/test_order_lifecycle.py \
		tests/unit/test_container.py

test-api-broker:
	@cd "$(API_DIR)" && uv run pytest -q \
		tests/unit/test_broker_models.py \
		tests/unit/test_broker_capabilities.py \
		tests/unit/test_broker_identity.py \
		tests/unit/test_broker_paper.py \
		tests/unit/test_broker_errors.py \
		tests/contract/test_broker_abstraction_contract.py \
		tests/integration/test_broker_oms_boundary.py \
		tests/unit/test_container.py
...
smoke-sprint3-runtime:
	@bash "$(ROOT)/scripts/smoke/smoke-sprint3-runtime.sh"
...
validate-sprint3-release:
	@bash "$(ROOT)/scripts/gates/validate-sprint3-evidence.sh" --release

test-sprint3-gate:
	@cd "$(API_DIR)" && uv run pytest -q "$(ROOT)/tests/gates/test_sprint3_gate.py"

gate-sprint3:
	@bash "$(ROOT)/scripts/gates/gate-sprint3.sh"
