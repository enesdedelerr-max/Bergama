"""Contract tests for Human Review Foundation."""
...
from app.human_review import __all__ as human_review_all
...
def test_human_review_models_are_frozen_and_forbid_extra() -> None:
    for model in (
        HumanReviewConfig,
        HumanReviewRecordedAttestation,
        HumanReviewRequest,
...
def test_human_review_output_contract_fields() -> None:
    ...
    assert set(HumanReviewOutput.model_fields) >= {
    ...
    assert set(HumanReviewUpstreamReferenceRecord.model_fields) >= {
        "sequence_index",
        "score_record_id",
        "instrument_key",
        "local_symbol",
        "score",
        "components",
        "dashboard_policy_version_id",
        "scoring_as_of",
    }
    assert set(HumanReviewProvenance.model_fields) >= {
        "policy_version_id",
        "ordering_preservation_policy_id",
        "presentation_preservation_policy_id",
        "human_attestation_policy_id",
    ...
    assert set(HumanReviewHistoryBinding.model_fields) >= {
        "human_review_output_id",
        "history_specification_id",
        "as_of",
        "upstream_dashboard_output_id",
...
def test_human_review_public_export_contract() -> None:
    required = {
        "POLICY_VERSION_V1",
        "ORDERING_PRESERVATION_POLICY_V1",
        "PRESENTATION_PRESERVATION_POLICY_V1",
    ...
    assert required <= set(human_review_all)
    assert POLICY_VERSION_V1 == "human-review.policy.v1"
    assert ORDERING_PRESERVATION_POLICY_V1 == "preserve_dashboard_order.v1"
    assert PRESENTATION_PRESERVATION_POLICY_V1 == ("include_all_dashboard_presentation_records.v1")
    assert HUMAN_ATTESTATION_POLICY_V1 == "explicit_human_attestation.recorded_input.v1"
    assert IDENTITY_SPECIFICATION_V1 == "human-review.identity.v1"
    assert PROVENANCE_SPECIFICATION_V1 == "human-review.provenance.v1"
    assert HISTORY_SPECIFICATION_V1 == "human-review.history.v1"
...
def test_human_review_models_reject_mutation() -> None:
    result = assemble_human_review(
        HumanReviewRequest(
            dashboard=_dashboard(),
            as_of=AS_OF,
    ...
    with pytest.raises(ValidationError):
        result.human_review_output_id = "0" * 64  # type: ignore[misc]
    with pytest.raises(ValidationError):
