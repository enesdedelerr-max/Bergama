"""Payload-safe Portfolio audit models and in-memory sink."""

from __future__ import annotations

from datetime import datetime

from pydantic import BaseModel, ConfigDict, Field, field_validator

from app.market_data.timing import require_utc_aware
from app.portfolio.identity import AccountId, PortfolioId
from app.portfolio.models import PortfolioMutationOutcome, PortfolioMutationType


class PortfolioAuditRecord(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)

    account_id: AccountId
    portfolio_id: PortfolioId
    mutation_type: PortfolioMutationType
    outcome: PortfolioMutationOutcome
    portfolio_version: int = Field(ge=0)
    idempotency_key_hash: str = Field(min_length=64, max_length=64)
    event_id: str = Field(min_length=1, max_length=128)
    correlation_id: str | None = Field(default=None, max_length=128)
    causation_id: str | None = Field(default=None, max_length=128)
    recorded_at: datetime
    error_code: str | None = Field(default=None, max_length=96)

    @field_validator("recorded_at")
    @classmethod
    def utc_recorded_at(cls, value: datetime) -> datetime:
        return require_utc_aware(value, field_name="recorded_at")


class InMemoryPortfolioAuditSink:
    def __init__(self, *, max_records: int = 1000) -> None:
        self._max_records = max_records
        self._records: list[PortfolioAuditRecord] = []

    def record(self, record: PortfolioAuditRecord) -> None:
        self._records.append(record)
        if len(self._records) > self._max_records:
            self._records = self._records[-self._max_records :]

    @property
    def records(self) -> tuple[PortfolioAuditRecord, ...]:
        return tuple(self._records)
