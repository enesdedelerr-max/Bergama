"""HTTP retry helper for Finnhub REST (Issue #304A)."""
...
class FailurePolicy(Protocol):
    """Thin policy surface used by Finnhub HTTP retries (future CB-ready)."""
    ...
    *,
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            last_error = FinnhubTimeoutError("finnhub request timed out")
            if attempt >= max_attempts:
                raise last_error from exc
            await sleeper(policy.delay_for_attempt(attempt))
            continue
        except httpx.TransportError as exc:
            last_error = FinnhubConnectionFailedError("finnhub connection failed")
            if attempt >= max_attempts:
                raise last_error from exc
            await sleeper(policy.delay_for_attempt(attempt))
            continue
        ...
        if attempt >= max_attempts:
            if response.status_code == 429:
                raise FinnhubRateLimitedError("finnhub rate limited")
            raise FinnhubProviderError(f"finnhub provider error status={response.status_code}")
    ...
    if last_error is not None:
        raise last_error
    raise FinnhubProviderError("finnhub request failed without response")
