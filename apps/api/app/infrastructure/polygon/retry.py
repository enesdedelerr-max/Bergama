"""HTTP retry helper for Polygon REST (no third-party retry library)."""
    ...
    *,
    ...
    for attempt in range(1, policy.max_attempts + 1):
        try:
            ...
            last_error = PolygonTimeoutError("polygon request timed out")
            if attempt >= policy.max_attempts:
                raise last_error from exc
            await sleeper(policy.delay_for_attempt(attempt))
            continue
        except httpx.TransportError as exc:
            last_error = PolygonConnectionFailedError("polygon connection failed")
            if attempt >= policy.max_attempts:
                raise last_error from exc
            await sleeper(policy.delay_for_attempt(attempt))
            continue
        ...
        if attempt >= policy.max_attempts:
            if response.status_code == 429:
                raise PolygonRateLimitedError("polygon rate limited")
            raise PolygonProviderError(f"polygon provider error status={response.status_code}")
    ...
    if last_error is not None:
        raise last_error
    raise PolygonProviderError("polygon request failed without response")
